...
public class VerticalOverScrollBounceEffectDecorator extends OverScrollBounceEffectDecoratorBase {
    ...
    protected static class MotionAttributesVertical extends MotionAttributes {
        ...
        public boolean init(View view, MotionEvent event, float xHistoryFirstDefault, float yHistoryFirstDefault) {
...
//            }
            ...
            float xHistoryFirst;
            if (event.getHistorySize() == 0) {
                if (xHistoryFirstDefault != -1 && yHistoryFirstDefault != -1) {
                    xHistoryFirst = xHistoryFirstDefault;
                    yHistoryFirst = yHistoryFirstDefault;
                } else {
                    return false;
                }
            } else {
                xHistoryFirst = event.getHistoricalX(0, 0);
                yHistoryFirst = event.getHistoricalY(0, 0);
            }

            // Allow for counter-orientation-direction operations (e.g. item swiping) to run fluently.
            final float dy = event.getY(0) - yHistoryFirst;
            final float dx = event.getX(0) - xHistoryFirst;
    ...
    public ValueTouch valueTouch;

    public VerticalOverScrollBounceEffectDecorator(IOverScrollDecoratorAdapter viewAdapter, ValueTouch valueTouch) {
        this(viewAdapter, DEFAULT_TOUCH_DRAG_MOVE_RATIO_FWD, DEFAULT_TOUCH_DRAG_MOVE_RATIO_BCK, DEFAULT_DECELERATE_FACTOR, valueTouch);
    }
    ...
    public VerticalOverScrollBounceEffectDecorator(IOverScrollDecoratorAdapter viewAdapter,
                                                   float touchDragRatioFwd, float touchDragRatioBck, float decelerateFactor, ValueTouch valueTouch) {
        super(viewAdapter, decelerateFactor, touchDragRatioFwd, touchDragRatioBck, valueTouch);
    }
