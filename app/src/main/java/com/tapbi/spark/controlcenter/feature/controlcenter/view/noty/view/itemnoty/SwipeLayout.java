...
import com.tapbi.spark.controlcenter.feature.controlcenter.adapter.AdapterChildNotyMi;
...
public class SwipeLayout extends FrameLayout implements View.OnTouchListener {
    ...
    private final Handler handlerDelete = new Handler(Looper.getMainLooper());
    ...
    private RecyclerView rccChild;
    private AdapterChildNotyMi adapterChildNotyMi;
    ...
    private TextView tv15Minutes, tv30Minutes, tv1Hours, tv1Day, name1, detailName1, name2, detailName2, tvNameNoty, tvTitle, tvMsg, textUndo, textTimeSnoozed, tvNameApp, tvCount, tvTimeNoty, tvTimeNotySetting, tvWaning, detailWaning, tvSilent, detailSilent, tvDone, tvOffNotification, textSnoozed;
    private DataNotiMi dataNotiMi;
    private int pos = 0;
    private final Runnable runnableDelete = () -> {
        try {
            if (pos >= dataNotiMi.notyGroups().size()) {
                pos = dataNotiMi.notyGroups().size() - 1;
            }
            deleteNotiGroup(dataNotiMi.notyGroups().get(pos).getNotyModels());
        } catch (Exception e) {
            Timber.e(e);
        }
    };
    ...
    public OnClickListener imgStateClick = new OnClickListener() {
        ...
        public void onClick(View v) {
            if (dataNotiMi.notyGroups().size() > pos && dataNotiMi.notyGroups().get(pos).getNotyModels().size() < 2) {
                expanedCollsnap();
            } else {
                setImgState(!showRecyclerView);
                if (showRecyclerView) {
                    tvTimeNoty.setVisibility(INVISIBLE);
                    groupDetail.setVisibility(GONE);
                    groupDetailExpand.setVisibility(GONE);
                    rccChild.setVisibility(VISIBLE);
    ...
    private OnClickListener clickItemSwipe = new OnClickListener() {
        @Override
    ...
    private void prepareChildView() {
        if (rccChild.getVisibility() == View.GONE) {
            rccChild.measure(View.MeasureSpec.UNSPECIFIED, View.MeasureSpec.UNSPECIFIED);
        }
    }
...
//            int bg_ripple_noty = R.drawable.bg_ripple_noty;
...
//
//            layoutNoti.setBackgroundResource(bg_ripple_noty);
//            layoutSnoozed.setBackgroundResource(bg_ripple_noty);
//            layoutSettingNoty.setBackgroundResource(bg_ripple_noty);
//            tvTimeNoty.setTextColor(color_gray_waning);
//            tvTimeNotySetting.setTextColor(color_gray_waning);
...
//            textSnoozed.setTextColor(color_gray_waning);
...
//            imgStateSnoozed.setColorFilter(color_gray_waning, mode);
//            lineSnoozed.setBackgroundColor(ContextCompat.getColor(getContext(), R.color.background_line));
...
//            tvTimeNotySetting.setTextColor(colorPrimary);
...
//            textSnoozed.setTextColor(colorPrimary);
...
//            imgStateSnoozed.setColorFilter(colorPrimary, mode);
//            lineSnoozed.setBackgroundColor(colorPrimary);
...
//        }
        ...
        tv1Hours.setBackgroundResource(bg_while_ripple_select_setting);
        setBgColor();
        setTypeFace(tv15Minutes, tv30Minutes, tv1Hours, tv1Day, name1, detailName1, name2, detailName2, tvNameNoty, tvTitle, tvMsg, textUndo, textTimeSnoozed, tvNameApp, tvCount, tvTimeNoty, tvTimeNotySetting, tvWaning, detailWaning, tvSilent, detailSilent, tvDone, tvOffNotification, textSnoozed);
    }
    ...
    public void setBgColor() {
        ...
        if (ThemeHelper.itemControl.getIdCategory() == Constant.VALUE_PIXEL) {
            if (ThemeHelper.itemControl.getPixel() != null && ThemeHelper.itemControl.getPixel().getBackgroundDefaultControl() != null) {
                color = ThemeHelper.itemControl.getPixel().getBackgroundDefaultControl();
            }
        }
    ...
    public void setCallBackUpdateHeight(DataNotiMi dataNotiMi, NotyCenterViewOS.OnNotyCenterCloseListener onNotyCenterCloseListener, int pos, NotyGroup notyGroup, ITouchItemView iTouchItemView) {
        this.dataNotiMi = dataNotiMi;
        this.pos = pos;
        this.notyGroup = notyGroup;
        this.onNotyCenterCloseListener = onNotyCenterCloseListener;
        this.iTouchItemView = iTouchItemView;
        if (notyGroup.getNotyModels().isEmpty()) return;
        notiModel = notyGroup.getNotyModels().get(0);
        resetLayout();
        setSateView(0);
        if (notiModel.getState() == NotyGroup.STATE.SNOOZED || notiModel.getState() == NotyGroup.STATE.EXPANDSNOOZED) {
            ...
            if (dataNotiMi.notyGroups().size() <= pos) {
                pos = dataNotiMi.notyGroups().size() - 1;
                this.pos = pos;
                return;
            }
            ...
            if (pos >= 0 && dataNotiMi.notyGroups().size() > pos && dataNotiMi.notyGroups().get(pos).getNotyModels().get(0).getImaBitmap() != null) {
                ConstraintLayout.LayoutParams layoutParams = (ConstraintLayout.LayoutParams) imgThumb.getLayoutParams();
                layoutParams.width = (int) DensityUtils.pxFromDp(getContext(), 38f);
                imgThumb.setLayoutParams(layoutParams);
                Bitmap bitmap = dataNotiMi.notyGroups().get(pos).getNotyModels().get(0).getImaBitmap();
                Glide.with(this).load(bitmap).into(imgThumb);
        ...
        adapterChildNotyMi = new AdapterChildNotyMi(rccChild, notyGroup, onNotyCenterCloseListener, iTouchItemView);
        rccChild.setLayoutManager(new NpaLinearLayoutManager(getContext(), RecyclerView.VERTICAL, false));
        rccChild.setHasFixedSize(false);
        rccChild.setAdapter(adapterChildNotyMi);
    ...
    private void setUpCountDown() {
        countDownTimer = new CountDownTimer(8000, 1000) {
            ...
            public void onTick(long millisUntilFinished) {
                Timber.e("millisUntilFinished: " + millisUntilFinished);
            }
            ...
            public void onFinish() {
                ...
                if (notyGroup != null) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        for (NotyModel notyModel : notyGroup.getNotyModels()) {
                            Timber.e("getKeyNoty: " + notyModel.getKeyNoty());
                            if (NotificationListener.getInstance() != null) {
                                NotificationListener.getInstance().snoozeNoti(notyModel.getKeyNoty(), DensityUtils.timeUnitToDuration(timeUnit, duration));
                            }
                        }
                    } else {
                        Data.Builder data = new Data.Builder();
                        NotyGroup notySnoozed = new NotyGroup();
                        notySnoozed.setState(notyGroup.getState());
                        notySnoozed.setGroupKey(notyGroup.getGroupKey());
                        notySnoozed.setPackageName(notyGroup.getPackageName());
                        //notySnoozed.setNotyModels(new ArrayList<>(notyModels));
                        notySnoozed.setNotyModels(notyGroup.getNotyModels());
                        ...
                        deleteNotiGroup(notySnoozed.getNotyModels());
                    }
                }
    ...
    private void setUpText() {
        ...
        tvTimeNoty.setText(getTimeAgo(getContext(), notiModel.getTime()));
        tvTimeNotySetting.setText(getTimeAgo(getContext(), notiModel.getTime()));
        int newCount = notyGroup.getNotyModels().size() - 2;
        tvCount.setText(newCount > 0 ? "+" + newCount : "");
        ...
        if (dataNotiMi.notyGroups().size() - 1 < pos) {
            Timber.e("dataNotiMi.notyGroups().size() - 1 < pos: " + pos);
            return;
        }

        if (dataNotiMi.notyGroups().get(pos).getNotyModels().size() > 1) {
            isShowIconExpaned = true;
            name1.setText(notiModel.getTitle());
            name2.setText(dataNotiMi.notyGroups().get(pos).getNotyModels().get(1).getTitle());
            detailName1.setText(notiModel.getContent());
            detailName2.setText(dataNotiMi.notyGroups().get(pos).getNotyModels().get(1).getContent());
            groupDetail.setVisibility(View.INVISIBLE);
            groupDetailExpand.setVisibility(View.VISIBLE);
        ...
        tvMsg.post(() -> {
            if (tvTitle == null || tvMsg == null || tvTitle.getLayout() == null || tvMsg.getLayout() == null) {
                return;
            }
    ...
    private void setUpView() {
        ...
        color_gray_waning = ContextCompat.getColor(getContext(), R.color.color_gray_waning);
    ...
    private void setClickOnOffNotification(boolean b) {
        ...
        int color_undo = ContextCompat.getColor(getContext(), R.color.color_undo);
        if (b) {
            ...
            tvWaning.setTextColor(color_undo);
            tvSilent.setTextColor(color_gray_waning);
            setTextViewDrawableColor(tvWaning, color_undo);
            ...
            tvSilent.setTextColor(color_undo);
            setTextViewDrawableColor(tvWaning, color_gray_waning);
            setTextViewDrawableColor(tvSilent, color_undo);
    ...
    private void findView() {
        ...
        rccChild = findViewById(R.id.rccChild);
        ...
        lineSnoozed = findViewById(R.id.lineSnoozed);
    ...
    public void resetLayout() {
        tvTimeNoty.setVisibility(VISIBLE);
        groupDetail.setVisibility(INVISIBLE);
        groupDetailExpand.setVisibility(VISIBLE);
        rccChild.setVisibility(GONE);
    ...
    private void addSwipeItems(int[] icons, int[] iconColors, int[] backgroundColors, String[] texts, int[] textColors,
        ...
        for (int i = 0; i < icons.length; i++) {
            ...
            swipeItem.setOnClickListener(clickItemSwipe);
    ...
    private void clickBySwipe() {
        handlerDelete.removeCallbacks(runnableDelete);
        handlerDelete.postDelayed(runnableDelete, 100);
    }

    private void deleteNotiGroup(ArrayList<NotyModel> list) {
        if (list.size() > 1) {
            String[] keys = new String[list.size()];
            for (int k = 0; k < list.size(); k++) {
                keys[k] = list.get(k).getKeyNoty();
            }
            if (NotificationListener.getInstance() != null) {
                Timber.e("hachung isDelete:"+list.get(0).isDelete());
                if (list.get(0).isDelete()) {
        ...
        if (swipeEnabled && rightIcons != null) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (Math.abs(prevRawX - event.getRawX()) < 10 && !movementStarted) {
                        if (System.currentTimeMillis() - lastTime >= 30 && !isPressed() && !isExpanding() && !longClickPerformed) {
                    ...
                    if (directionLeft) {
                        ...
                        //Timber.e("hoangld: left: " + left + " /iTouchItemView: "+iTouchItemView);
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (movementStarted) {
                        ...
                        view.setPressed(false);
                        //Timber.e("System.currentTimeMillis() - downTime: " + (System.currentTimeMillis() - downTime) + " ViewConfiguration.getTapTimeout(): " + ViewConfiguration.getTapTimeout() + " System.currentTimeMillis(): " + System.currentTimeMillis() + " downtime: " + downTime + " ViewConfiguration.getTapTimeout(): " + ViewConfiguration.getTapTimeout());
    ...
    public void removeItemChild(int posChild) {
        Timber.e("xoa o group keyNoty: " + " / " + posChild);
        if (posChild != -1) {
            adapterChildNotyMi.removeNotyInAdapter(posChild);
            updateTextPreview(0);
        }
    }
    ...
    public void notifyItemChild(int pos) {
        updateTextPreview(0);
        if (adapterChildNotyMi == null) {
            return;
        }
        rccChild.post(() -> {
            if (adapterChildNotyMi.isPosValid(pos)) {
                adapterChildNotyMi.notifyItemChanged(pos);
            }
        });
    }
...
//        }
        ...
        if (notyGroup.getNotyModels().size() > 0 && adapterChildNotyMi != null) {
            adapterChildNotyMi.insertNoty();
        }
    }
    ...
    public void removeItemChildWhenAdd(int pos) {
        ...
        rccChild.post(() -> {
            try {
                adapterChildNotyMi.notifyItemRemoved(pos);
            } catch (Exception e) {
            }
        });
    }

    void updateTextPreview(int pos) {
        if (!NotyManager.INSTANCE.getListNotyGroup().isEmpty()) {
            notyGroup = NotyManager.INSTANCE.getListNotyGroup().get(0);
        }
        if (pos == 0 && pos < notyGroup.getNotyModels().size()) {
            notiModel = notyGroup.getNotyModels().get(pos);
        }
        if (rccChild.getVisibility() != View.VISIBLE) {
            setUpText();
        }
    }
