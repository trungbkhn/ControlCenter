...
class SSCustomBottomNavigation : FrameLayout {
    ...
    private var shadowColor = Color.parseColor("#EBF1FF")
    ...
    private val paddingHorizontal = DimensionHelper.dpToPx(6)
    ...
    init {
        heightCell = DimensionHelper.dpToPx(95) // bottom navigation height
    }
    ...
    private fun initializeViews() {
        ...
        addView(bezierView)
        ll_cells.setPadding(paddingHorizontal, 0, paddingHorizontal, 0)
        addView(ll_cells)
        allowDraw = true
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (selectedIndex != -1) {
            ...
            Log.d("chungvv", "onMeasure: " + selectedIndex)
            show(selectedIndex, false)
        }
    }
        ...
        this.cbnMenuItems.forEachIndexed { _, cbnMenuItem ->
            add(
                Model(
                    ...
                    allowShowView = cbnMenuItem.allowShowView
                )
            )
        }
    }
...
//    }
    ...
    fun show(id: Int, enableAnimation: Boolean = true) {
        var allowAnim = true
        Log.d("chungvv", "show id: " + id)
        for (i in models.indices) {
            val model = models[i]
            Log.e("chungvv:", "model: " + model.id + " allowShowView: " + model.allowShowView)
            if (model.id == id) {
                allowAnim = model.allowShowView
            }
        }
        Log.d("chungvv", "show: " + allowAnim)
        if (allowAnim) {
            for (i in models.indices) {
                val model = models[i]
                val cell = cells[i]
