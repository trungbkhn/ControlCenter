...
public class SimpleRatingBar extends View {
  ...
  private void parseAttrs(AttributeSet attrs) {
    ...
    borderColor = arr.getColor(R.styleable.SimpleRatingBar_srb_borderColor, Color.BLACK);
  ...
  private void drawFromLeftToRight(Canvas internalCanvas) {
    ...
    for (int i = 0; i < numberOfStars; i++) {
      if (remainingTotalRating >= 1) {
        drawStar(internalCanvas, startingX+40, startingY, 1f, Gravity.Left);
        remainingTotalRating -= 1;
      } else {
        drawStar(internalCanvas, startingX+40, startingY, remainingTotalRating, Gravity.Left);
